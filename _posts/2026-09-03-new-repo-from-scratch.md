...
## 5. ~~저장소 옆 `Deployments` 의 숫자는 배포 횟수가 아니다~~ → **배포 횟수가 맞다**

> **2026-09-07 정정.** 이 절의 설명이 틀렸습니다.
> 줄이 그어진 부분이 처음에 잘못 쓴 것이고, 그 아래가 맞는 내용입니다.
{: .prompt-warning }
...
→ **옆의 숫자는 배포 횟수다.** 나중에 이 블로그 저장소로 다시 확인해보니
environments 1개, deployments 18건이었고, **화면에 뜬 숫자는 18** 이었다.

~~Pages를 켜면 `github-pages` 라는 환경이 자동으로 하나 생기고,~~
~~이후 push마다 **같은 환경에 새 배포를 덮어쓴다.** 목적지가 하나니까 계속 `1` 이다.~~

→ 환경이 하나 생기고 같은 환경에 덮어쓰는 것까지는 맞다.
다만 화면의 숫자가 그 환경 개수를 가리키는 것은 아니었다.
그때 `1` 로 보인 이유는 다시 확인하지 못했다.
...
- ~~`Deployments` 숫자는 환경 개수다. 배포 횟수가 아니다~~ → **배포 횟수가 맞다** (2026-09-07 정정)
